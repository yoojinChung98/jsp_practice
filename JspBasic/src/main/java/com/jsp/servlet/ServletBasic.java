...
public class ServletBasic extends HttpServlet {
	...
	//GenericServlet이 HttpServlet의 부모.
	@Override
	public void init() throws ServletException {
		/*
        - 페이지 요청이 들어왔을 때 처음 실행할 로직을 작성.
        - init()은 컨테이너(서버 프로그램(톰캣같은))에 의해 서블릿 객체가 생성될 때
        최초 1회 자동으로 호출됩니다.
        - 객체의 생성자와 비슷한 역할을 수행합니다.
        
        -필요하면 뽑고 필요 없으면 안만들면 됨.
        -따로 호출하는 것이 아닌, 조건에만 맞으면 자동으로 호출되는 것.
        */
...
//		/*
//		httpservletRequest 는 요청에 대한 모든 정보를 가지고 있는 객체임.
//		 * 
//        - 페이지 요청이 들어왔을 때 자동으로 호출.
//        - get, post 구분 없이 들어오는 모든 요청과 응답을 관제할 수 있다.
//    - 매개값으로 요청, 응답 내장 객체가 전달됩니다.
//    
//     -톰캣이 이 메서드를 부르는거고, 필요한 건 알아서 챙겨간다는데...
//        */
//		
//		//service객체가 자동으로 생성되는지 한번 체크해보자!
//		System.out.println("service 메서드가 호출됨!");
//		
//		
//		/*
//		//요청에 관련된 모든 내용은 자동으로 생성된 내장객체 HttpServletRequest가 모든 것을 갖고 있으므로 얘한테 다 물어봐!
//		//요청 방식 묻기
...
//		*/
//		
//		//queryString이 다 붙어있는데, 값을 따로 받는 방법
...
//		
//		//우선 내가 사용자에게 응답하고자하는 컨텐트의 타입을 설정해줘야 함. (텍스트로 이루어진 html, 한글쓸꺼니까 인코딩은 utf-8)
//		response.setContentType("text/html");
//		response.setCharacterEncoding("UTF-8");
//		
//		//이게 바로 서블릿클래스에서 작성하는 것. 톰캣이 자동으로 해줄 수 있는 것.
//		//응답 화면 제작
//		//자바 클래스에서 브라우저로 바로 응답을 구현하기 위해서는
//		//printWritter 객체를 사용.
//		//웹 프로그래밍에서는 기본적으로 제작된 객체를 받아서 사용하는 경우가 점점 많아질 것.
//		
//		//리스펀스야, 나 프린트라이터 객체좀 하나만...ㅎㅎ 클래스에서 브라우저로 보낼 때 사용하는 애
...
//	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//post방식으로 요청이 들어왔을 때, 함께 전송된 파라미터를 얻으실 때는 인코딩 작업을 다시 거치셔야 합니다!
